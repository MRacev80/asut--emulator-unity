...
Рабочая доска задач: [TASKS.md](TASKS.md) | Бизнес-процесс: [docs/business-process.md](docs/business-process.md) | Онбординг: [ONBOARDING.md](ONBOARDING.md)
...
|-----|-----------|-----------|
| 1. Spec | `/asutп-spec` — Claude задаёт вопросы, заполняет 11-секционный шаблон | Формализованное ТЗ на FB (Identity, I/O, Интерлоки, stateDiagram, FAT) |
| 2. Генерация ST | Claude читает spec → `create_pou` + `set_pou_code` → `compile_project` | Скомпилированный FB без ошибок |
| 3. Загрузка | `download_to_plc` (simulationMode=False) → `get_application_state` | Control Win V3 работает, OPC UA Server активен |
...
| Claude Agent | АСУТП Emulator Engineer (`~/.claude/agents/`) | Авто при релевантных задачах |
| Claude Skill | `/asutп-spec` (`asutп-spec/SKILL.md`) | `/asutп-spec` в чате |
...
```
specs/
  _шаблон/equipment-spec.md  — универсальный шаблон (11 секций)
  uvelka/FB_Shnek.md          — заполненный spec шнека дозирования
asutп-spec/SKILL.md           — скилл /asutп-spec для Claude
python-bridge/
  bridge.py                   — OpcUaSource + WsServer
  tags.yaml                   — реестр OPC UA тегов
  tests/                      — FAT автотесты (pytest + asyncua)
unity/                        — Unity проект AsutpEmulator
docs/                         — диаграммы, отчёты FAT
CLAUDE.md                     — инструкции и MCP справочник для Claude
ONBOARDING.md                 — онбординг нового сотрудника
setup.bat                     — автоматическая настройка окружения (шаги 2-5)
verify.py                     — проверка стека (PASS/FAIL таблица)
...
### 6.4 Шаблон спецификации оборудования — 11 секций

Каждый FB описывается в `specs/объект/FB_Имя.md`. Скилл `/asutп-spec` заполняет шаблон интерактивно.

| Секция | Что содержит | Используется для |
|--------|-------------|-----------------|
| 1. Карточка | P&ID тег, тип, местоположение, роль | Документация |
| 2. Контекст | Upstream/downstream, режим работы | Документация |
| 3. Режимы | LOCAL/REMOTE/MAINT/FAULT | `nMode` в ST-коде |
| 4. Физические I/O | DI/AI/DO/AO с тегами (HS-101-RUN, IE-101, KA-101) | `VAR_INPUT`/`VAR_OUTPUT` в ST |
| 5. Soft I/O | bCmdStart, bRunning, nState и др. | FB интерфейс |
| 6. Блокировки | Пермиссивы (AND) + защиты (немедленный останов) | Условия в state machine |
| 7. stateDiagram | STOPPED→STARTING→RUNNING→STOPPING→FAULT | `CASE nState OF` |
| 8. Таймеры | tRunFeedbackTO, tStopFeedbackTO и др. | `TON` таймеры в ST |
| 9. Аварии | Коды A-N-01..N, сообщения, сброс | `bFault*` флаги |
| 10. HMI | Цвета состояний, OPC UA теги для SCADA | tags.yaml |
| 11. FAT | Сценарии с предусловием/действием/ожидаемым результатом | pytest тест-кейсы |

**Соглашение по тегам (ISA-5.1):**
```
DI обратная связь:  HS-[N]-RUN      (1=работает)
DI тепловая защита: HS-[N]-THERM    (1=OK, НЗ)
DI местный/дистанц: HS-[N]-LR       (1=дистанц.)
AI ток двигателя:   IE-[N]          (4-20мА = 0-Iном А)
DO команда пуска:   KA-[N]          (катушка контактора)
AO задание ЧРП:     SE-[N]          (4-20мА = 0-50Гц)

Где [N] = числовой номер из P&ID тега (M-101 → N=101)
```

### 6.5 Шаблон ST Function Block

Стандартная структура, генерируемая из spec.md:
...
VAR_INPUT
    ...
    bRunFeedback : BOOL;    (* OS raboty *)
    bThermalOK   : BOOL;    (* Teplovaya zashchita OK=TRUE *)
    (* Interlocks from other FBs *)
    bInterlockOK : BOOL;    (* Razreshenie ot sosednikh FB *)
END_VAR
VAR_OUTPUT
    bRunning     : BOOL;    (* Rabotaet *)
    bFault       : BOOL;    (* Avariya aktivna *)
    bReady       : BOOL;    (* Gotov k pusku *)
    nState       : INT;     (* 0=Stop 1=Start 2=Run 3=Stopping 4=Fault *)
    bStartCmd    : BOOL;    (* Komanda na kontaktor *)
END_VAR
VAR
    STATE_STOPPED  : INT := 0;
...
- **Комментарии ТОЛЬКО транслитом** — `// Sbros avarii`, не `// Сброс аварии`
- State machine через `CASE nState OF ... END_CASE`
- Edge detection через `bStartEdge := bCmdStart AND NOT bLastStart`
- Все таймеры из секции 8 spec, все интерлоки из секции 6

### 6.6 Добавление нового тега в стек
...
1. **Spec**: написать `/asutп-spec` → Claude заполнит `specs/объект/FB_Имя.md` (11 секций)
2. **ST**: Claude генерирует FB через `create_pou` + `set_pou_code` → `compile_project`
3. **Deploy**: `download_to_plc` (simulationMode=False) → `get_application_state` = Running
4. **OPC UA**: добавить переменные в `GVL_HMI` → переменные публикуются автоматически (wildcard)
5. **tags.yaml**: добавить теги в `python-bridge/tags.yaml`
6. **Unity**: добавить GameObject + C#-скрипт, подписаться на теги через `PLCBridge`
7. **FAT**: Claude генерирует `tests/test_fb_имя.py` из секции 11 spec → `pytest`
...
- ✅ T-003 Шаблон spec.md (минимальный → заменён T-005)

### MCP расширения ✅

- ✅ T-MCP-08 `read_pou_code` — читает declaration + implementation любого POU
- ✅ T-MCP-09 `get_application_state` — Running/Stopped/Error/NoApplication
- ✅ T-MCP-11 `create_gvl` — создаёт Global Variable List с кодом
- ✅ T-MCP-12 `create_dut` — создаёт STRUCT/ENUM/UNION
- ✅ T-MCP-13 `list_project_objects` — дерево объектов проекта
- ✅ T-MCP-14 `start_stop_application` — старт/стоп/сброс без перезагрузки
- ✅ `diagnose_symbol_config` — диагностика объекта Symbol Config
- ~~ `update_symbol_configuration` — SP17: нет API. **Workaround: GVL_HMI + wildcard** ~~
...
### Инструментарий и онбординг ✅

- ✅ T-005 Шаблон `specs/_шаблон/equipment-spec.md` — 11 секций, hints для Claude, YAML профили
- ✅ T-007 stateDiagram + аварийные ситуации — в секциях 7 и 9 шаблона
- ✅ T-AGENT-01 Claude агент `АСУТП Emulator Engineer` — SP17 quirks, MCP таблица, ST шаблоны
- ✅ T-SKILL-01 Claude скилл `/asutп-spec` — интерактивное создание spec.md из описания
- ✅ T-ONBOARD-01 `ONBOARDING.md` — 6 шагов от нуля до рабочего окружения
- ✅ T-SETUP-01 `setup.bat` + `verify.py` — автоматическая установка и проверка стека
...
- ⬜ T-009 Полный spec.md для FB_Shnek — запустить `/asutп-spec`
- ⬜ T-012 Claude генерирует FB_Shnek через MCP
- ⬜ T-013 OPC UA теги FB_Shnek (через GVL_HMI)
- ⬜ T-014 Unity сцена П&ИД шнека
- ⬜ T-015 PLCBridge.cs двусторонняя связь шнека
- ⬜ T-016 FAT-скрипт шнека (5 тестов из секции 11 spec)
